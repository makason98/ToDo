<x-admin-layout>
    ...
    <div class="flex h-[calc(100vh-64px)]" x-data="{ sidebarOpen: false }">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            @include('admin.partials.sidebar', ['currentPage' => 'trash'])
        </aside>
        ...
        <main class="flex-1 overflow-y-auto">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
                ...
                <div class="flex items-center gap-1 mb-4 border-b border-gray-200 dark:border-gray-700">
                    <a href="{{ route('admin.trash.index', ['type' => 'tasks']) }}"
                        class="px-4 py-2 text-sm font-medium border-b-2 -mb-px {{ $type === 'tasks' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' }}">
                        Tasks <span class="ml-1 text-xs px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full">{{ $counts['tasks'] }}</span>
                    </a>
                    <a href="{{ route('admin.trash.index', ['type' => 'comments']) }}"
                        class="px-4 py-2 text-sm font-medium border-b-2 -mb-px {{ $type === 'comments' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' }}">
                        Comments <span class="ml-1 text-xs px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full">{{ $counts['comments'] }}</span>
                    </a>
                </div>

                <!-- Empty trash button -->
                @if(($type === 'tasks' && $counts['tasks'] > 0) || ($type === 'comments' && $counts['comments'] > 0))
                    <form method="POST" action="{{ route('admin.trash.empty') }}" class="mb-4"
                        onsubmit="return confirm('Permanently delete ALL trashed {{ $type }}? This cannot be undone.')">
                        @csrf
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                    @forelse($tasks as $task)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                            <td class="px-4 py-3 text-gray-900 dark:text-gray-100">{{ $task->title }}</td>
                                            ...
                                            <td class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 whitespace-nowrap" title="{{ $task->deleted_at }}">{{ $task->deleted_at->diffForHumans() }}</td>
                                            <td class="px-4 py-3">
                                                <div class="flex items-center justify-center gap-2">
                                                    <form method="POST" action="{{ route('admin.trash.tasks.restore', $task->id) }}">
                                                        @csrf
                                                    ...
                                                    <form method="POST" action="{{ route('admin.trash.tasks.purge', $task->id) }}"
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                    @forelse($comments as $comment)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                            ...
                                            <td class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 whitespace-nowrap" title="{{ $comment->deleted_at }}">{{ $comment->deleted_at->diffForHumans() }}</td>
                                            <td class="px-4 py-3">
                                                <div class="flex items-center justify-center gap-2">
                                                    <form method="POST" action="{{ route('admin.trash.comments.restore', $comment->id) }}">
                                                        @csrf
                                                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-green-500 hover:bg-green-600 text-white rounded-lg shadow-sm transition-all">
                                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
                                                            Restore
                                                    ...
                                                    <form method="POST" action="{{ route('admin.trash.comments.purge', $comment->id) }}"
